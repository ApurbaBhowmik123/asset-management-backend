...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        const user = yield prisma_1.default.user.findFirst({ where: { status: true } });
        if (!user) {
            console.log("No user found");
            return;
        }
        const token = jsonwebtoken_1.default.sign({ id: user.id }, process.env.JWT_SECRET || 'secret', { expiresIn: '1h' });
        const response = yield fetch("http://localhost:8000/api/gr/inventory/filter/SCRAP?page=1&limit=10", {
            headers: {
